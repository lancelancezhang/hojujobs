...
import { Search, ArrowUpDown, Plus, LogIn, LogOut, FileText, Shield } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      <header className="bg-primary text-primary-foreground">
        <div className="max-w-6xl mx-auto px-4 py-4 sm:py-6">
              ...
              {user ? (
                <>
                  <Button variant="secondary" size="sm" onClick={() => navigate("/post-job")} className="gap-1 sm:gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
                    <Plus className="h-3.5 w-3.5" /> <span className="hidden sm:inline">공고 등록</span><span className="sm:hidden">등록</span>
                  </Button>
                  <Button variant="secondary" size="sm" onClick={() => navigate("/my-posts")} className="gap-1 sm:gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
                    <FileText className="h-3.5 w-3.5" /> <span className="hidden sm:inline">내 공고</span><span className="sm:hidden">내 글</span>
                  </Button>
                  {isAdmin && (
                    <Button variant="secondary" size="sm" onClick={() => navigate("/admin")} className="gap-1 sm:gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
                      <Shield className="h-3.5 w-3.5" /> <span className="hidden sm:inline">관리</span><span className="sm:hidden">관리</span>
                    </Button>
                  )}
                  <Button variant="ghost" size="sm" onClick={signOut} className="text-primary-foreground/80 hover:text-primary-foreground hover:bg-primary-foreground/10 px-2 sm:px-3 gap-1">
          ...
          <p className="text-xs sm:text-base opacity-90 mt-1">🇦🇺 호주 한인 구인구직 게시판</p>
        </div>
      </header>
